...
test("compact gameplay and safe centering hold in rendered Chrome", { timeout: 150000 }, async (t) => {
  ...
  try {
    ...
    await t.test("the topic screen scrolls inside its card and keeps its touch targets", async () => {
    ...
    await t.test("Binyanim root tiles stay compact on the tallest draw", async () => {
    ...
    await t.test("a Binyanim question centers its prompt and clears the footer", async () => {
    ...
    await t.test("the companion does not drift during an answer pulse", async () => {
    ...
    await t.test("sentence-bank feedback fits the 360px floor", async () => {
    ...
    await t.test("Shema feedback fits the 360px floor", async () => {
    ...
    await t.test("the feedback tray keeps even padding across widths", async () => {
    ...
    await t.test("the handwriting canvas keeps its size when feedback opens", async () => {
      await evaluate(pageCdp, `(() => {
    ...
    await t.test("the handwriting results grid keeps three legible columns", async () => {
      await evaluate(pageCdp, `(() => {
    ...
    await t.test("abbreviation results keep their labeled fields legible", async () => {
      await evaluate(pageCdp, `(() => {
        IvriQuestApp.session.showSessionSummary({
          game: 'abbrMatch',
          correctCount: 17,
    ...
    await t.test("mission results keep the hero tight under the title", async () => {
    ...
    await t.test("widescreen gameplay stays vertically centered", async () => {
      await pageCdp.send("Emulation.setDeviceMetricsOverride", {
        width: 1366,
        height: 768,
        deviceScaleFactor: 1,
    ...
    await t.test("settings centers while it fits", async () => {
      await pageCdp.send("Emulation.setDeviceMetricsOverride", {
        width: 1366,
        height: 1000,
        deviceScaleFactor: 1,
    ...
    await t.test("compact settings stays scrollable with a reachable top", async () => {
      await pageCdp.send("Emulation.setDeviceMetricsOverride", {
        width: 360,
        height: 640,
        deviceScaleFactor: 1,
