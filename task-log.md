...
**Files changed:**
- `app/character-data.js` — the withholding layer. New `CAST_WIDE_SENTENCE_IDS` (21 ids) and `characterData.SHARED_ITEM_IDS`, keyed by kind, which un-fence the ordinary civilian-safety rows inside Idan's bank — the fire extinguisher, the shelter being in the yard, the crosswalk, the municipal water outage, the bag check at the mall, checking the safe room monthly. New `route.sentenceReserveIds` on Inat (34 ids) covering the political tranche that was authored into the `colloquial`, `everyday`, `professional` and `formal` banks; reserving both grants and fences, so those rows are hers now rather than unowned filler. New `route.vocabReserveCategories` on Idan (`military_operational`, `emergency_response`) and Inat (`politics_society_expanded`), `route.vocabReserveWords` on Idan (the hard-security and trauma subset of his existing `vocabWords`), `route.abbrReserveIds` on Idan (`MILITARY_ABBR_IDS`), and `route.verbReserveIds` on Idan (`להרוג`, `לחסל`, `לפוצץ`). `ownsItem` moved here from `app/character.js` so the report's duplicate copy could be deleted, and its sentence branch now also reads `sentenceReserveIds`. New `getItemAudience(kind, item)` with the three-step precedence documented in the strategy doc.
- `app/character.js` — `ownsItem` delegates to the registry. New `character.isContentWithheld(kind, item)` and `character.filterWithheldContent(kind, items, { getItem, isSeen })`. The weigher, the boost math and the bond-XP path are untouched.
- `app/data.js` — new `data.hasWordProgress(wordId)` (counts `attempts`, `conjugationAttempts`, `mastered`, because a card first met in Conjugation has no `attempts`). `pickBestWord` filters before `getDueWords`.
- `app/sentence-bank.js` — new `sentenceBank.hasSentenceProgress(sentenceId)`, checked per sentence rather than per direction. `buildCandidatePairs` filters the pool first and both existing empty-pool fallbacks now fall back to the *filtered* array. `buildSentenceBankReviewQuestion` carries a comment recording that it is deliberately unfiltered.
- `app/abbreviation.js`, `app/verb-match.js`, `app/handwriting.js` — the same filter at each draw site. Handwriting filters `usable` (the 542-row letter-window set) rather than the shortlist, so the safety valve stays far from firing. The verb site is close to a no-op today and says so.
- `scripts/character-content-report.js` — deleted its duplicate `ownsItem`, now calls the registry's. Added a `reserved` footer row and a second "Draw pool after withholding" table.
- `docs/character-gameplay-strategy.md` — new "Content withholding" section: the three-step precedence, what deliberately does not fence and the two measurements behind that, why `civil_defense_safety` and review are exempt, why `abbrExcludeIds` must stay, and the implementation traps. Idan's and Inat's sections updated, plus a new first item under "Next decisions" for the authoring priority.
- `docs/sentence-bank-authoring.md` — cross-reference to the new section.
- `index.html` — bumped `?v=20260807a` for the seven changed `app/*.js` files.
- `tests/character-mission.test.js` — 9 new tests.

**Behavior changed:** Content strongly coded to one character is no longer served to the others as new material. Measured: 224 of 717 sentences, 341 of 2108 vocabulary cards, 26 of 282 abbreviations and 3 of 247 verbs are now fenced. Previously all 90 `idan_` sentences carried `category: "everyday"`, so sirens, shrapnel, a call-up order and pressing on a wound were *unowned* rather than withheld and surfaced in every other character's mission; the political tranche leaked the same way through other characters' register banks; and Ido and Inbal drew צה״ל because `abbrExcludeIds` had only ever been applied to the two bucket holders. Register stays shared as requested — a `colloquial`, `everyday`, `professional` or `formal` row is never fenced, and neither are ordinary topic shelves. `civil_defense_safety` vocabulary stays cast-wide. Review is exempt: an item the learner already met returns under any character with its Leitner interval intact. Free play still draws all 717 sentences. Inat's owned sentences went 132 → 152; Ivri still owns 113 by register but can draw 102 of them, because Inat reserves eleven `professional_` rows.

**Tests run:**
- `npm test` — **380 pass, 0 fail** (371 before; +9 new tests). Baseline 371 pass recorded before any edit.
- New tests: strongly coded content withheld from non-owners and never from its owner; register, style, buckets, verbs and ordinary topic shelves never withheld, asserted across all five characters; the allow-list un-fences the mild safety rows while their alarming neighbours stay fenced, and the allow-list and reserve lists are disjoint; a reserved row in a shared register bank becomes its reserver's alone *and* is owned by them; the doc's deliberate Inbal/Ido multi-owner colloquial case survives; already-met content is exempt, the empty-pool safety valve returns the input, and free play withholds nothing; every allow-listed and reserved id resolves to a real sentence; a draw-pool floor canary per character per pool; and a wiring test pinning the filter at all five draw sites (the app-progress harness does not load the character modules).
- Browser walkthrough at 1280×720 on `npm start`: no console errors; character picker, mission picker, greeting, mission hub and the Vocabulary board all render. Drove the production pickers directly — 4000 sentence draws as Ido reached 521 distinct rows with zero political leaks and the only prefixed rows being the 21 allow-listed plus Inbal's 28 shared colloquial; as Idan, 562 distinct with all 90 own rows reachable and zero Inat/Inbal rows; free play reached all 717 including the siren and the coalition row; a met siren row returned under Ido while an unmet one stayed fenced. 5000 vocabulary draws as Ido reached no `military_operational`, `emergency_response` or `politics_society_expanded` card, kept `civil_defense_safety` and `groceries_food`, and did not leak שירות מילואים despite it sitting on his own shelf. 4000 abbreviation draws as Ido: zero military leaks, ממ״ד still reachable. Handwriting: 542 eligible rows, 377–450 allowed per character, zero leaks, 62–170 owned-and-eligible each against a 1-round activity.
- `npm run report:characters` before and after — recorded in the two tables above.

**Risks / regressions to check:** The realized owned-draw share moved *up* rather than down (Idan 0.52 → 0.56, Ido 0.55 → 0.59, Inat 0.56 → 0.62, measured by bypassing the filter in the live page), so character focus improved; the remaining gap to `TARGET_OWNED_SHARE = 0.65` is pre-existing and comes from the eight other multiplicative boosts. `abbrExcludeIds` on Ivri and Inat must **not** be deleted as redundant — it is the input that makes `MILITARY_ABBR_IDS` single-owner and therefore fenceable. `getItemAudience` is derived at call time by scanning all five routes; it is well inside budget today but is the place to memoize if vocabulary drawing ever feels slow. Reservation data cannot move onto a content row, because `prepareSentenceBankDeck` whitelists the fields it copies. Adding a sixth character will instantly un-fence anything they co-own. `app/session.js:59`'s restore-by-id lookup is deliberately unfiltered — filtering it would make a restored mid-mission question look stale. The report's ownership column now reads eleven higher than Ivri's drawable pool, which is noted in the doc.
...
**Why deletion rather than wiring it in:** `grep -rn '\.level\b'` across `app/` finds only Leitner box levels (`app/data.js`, `app/sentence-bank.js`, `app/abbreviation.js`) and character bond levels (`app/character.js`) — none is this field. The single reader anywhere was `tests/hebrew-idioms.test.js:152`, asserting a level-4 entry existed, which proved the data was present and never that anything consumed it. So the ladder cost an authoring judgement on every new idiom, that judgement was discarded, and nothing validated that level 2 was in fact harder than level 1 — it could drift indefinitely without a signal. `pickAdvConjQuestions` already weights by per-idiom accuracy, which models "hard for this learner" rather than "hard in the abstract", and is the better signal for a drill.

**Files changed:**
- `hebrew-idioms.js` — removed the `"level"` line from all 81 entries. Distribution was 6 / 40 / 24 / 11 across levels 1–4.
- `tests/hebrew-idioms.test.js` — deleted the level-4 assertion and its comment; replaced with a test that no entry carries `level`, so the field cannot creep back in via a copied template. Recorded the reasoning in a comment above it, since "this used to exist and was deliberately dropped" is exactly what a future author would otherwise have to rediscover.

**Behavior changed:** None. Verified in the browser: 81 idioms, 0 carrying `level`, deck still builds ~12,953 items with four options on every item, no console errors. Byte-for-byte the same gameplay — which is the point, and the evidence the field was inert.

**Tests run:** `npm test` — **346 pass, 0 fail, 0 skipped** (345 before; net +1 from replacing one assertion with a standalone test).

**Risks / regressions to check:** (1) `hebrew-idioms.js` is already cache-busted to `?v=20260731a` from earlier in the session and unpushed, so that version covers this edit too. (2) If a difficulty ladder is ever wanted again, it should be introduced together with the code that consumes it — most naturally as a prior inside `getAdaptiveWeight` for unseen items, which currently give every idiom a flat 1.45× boost regardless of how hard it is. Reintroducing the data first is what produced this situation.


### 2026-07-31 19:05 EDT — Close both Conjugation+ axes completely: every idiom person-marked, feminine plurals reachable

**Requested:** Continue the two gaps left open by the previous entry — the 38 idioms still limited to third person in past and future, and the `2fpl`/`3fpl` objects that no paradigm could supply.

**Files changed:**

- `preposition-data.js` — added `2fp` (לָכֶן / אֶתְכֶן) and `3fp` (לָהֶן / אוֹתָן) to the `le` and `et` paradigms. **Only those two.** `PREPOSITION_OBJECTS` is untouched, so the Prepositions deck keeps its eight objects and its item count; the other fourteen paradigms would be authoring rows nothing reads. The shape test iterates a fixed list of eight keys and asserts presence, not exact membership, so extra rows are safe.
- `app/constants.js` — `ADV_CONJ_OBJECT_KEYS` gains `2fpl` and `3fpl`, taking the object axis from 8 to **10**. Plural labels now carry gender (`you (m.pl.)`, `them (m.)`) for the same reason the singular did once `2fsg` arrived: a bare "you (pl.)" reads onto both אֶתְכֶם and אֶתְכֶן.
- `hebrew-verbs.js` — **34 new paradigms, 197 → 231**, one for every lemma that still had no paradigm behind it. Each modelled on an in-repo entry of the same shape rather than pointed from scratch: paal regular on `לשמור`; final-guttural paal (`לקרוע`, `לקטוע`, `למרוח`) with the patach present-fs and a-vowel future; R2-guttural paal (`לסחוב`, `לדחוף`) with hataf plurals; lamed-hey paal (`לזרות`) on `לשתות`; geminate/hollow piel (`לסובב`, `לפוצץ`) on the `לעודד` polel shape; R2-resh piel (`למרר`, `לפרק`) on `לברר`, where the dagesh cannot land and the vowel lengthens; hollow hifil (`להרים`, `להעיף`, `להאיר`) on `להביא`; pe-nun hifil (`להפיל`, `להטות`) on `להציל`; guttural-pe hifil (`להעביר`, `להעלות`) on `להעליב`. Six lemmas had verified pointing already in the repo and were anchored to it — that is how `לשרוף` came out as **לִשְׂרוֹף with sin**, which a pattern derivation would have got wrong, and `להפוך` as `לַהֲפוֹךְ`. All 34 are registered in `TRANSLATION_HIDDEN_STARTER_VERB_IDS`: they exist to give the idioms person-marked forms, not as vocabulary, and Translation Match already carries cards for these meanings.
- `index.html` — `preposition-data.js` cache-busted to `?v=20260731a`, joining the six files already bumped.
- `tests/hebrew-verbs.test.js` — entry count 197 → 231.
- `tests/hebrew-idioms.test.js` — object-key list gains `2fpl`/`3fpl`; the join coverage floor goes 43 → **81**, which is now every idiom.
- `tests/app-progress.test.js` — three more label expectations updated from `(pl.)` to `(m.pl.)`.

**Behavior changed:** **Every one of the 81 idioms now conjugates first and second person in past and future** — the join has no unlinked entries left, so the coverage caveat in the previous entry is closed rather than reduced. The object axis reaches all ten pronouns, so `אֶתְכֶן` and `אוֹתָן` are drillable and the `2fpl`/`3fpl` suffix forms authored on all 16 `possessive_suffix` entries are finally reachable — 2,793 deck items use a feminine-plural object. Deck **8,400 → 12,955 items**, and past (4,981) now outnumbers present (3,733), which is the clearest single sign the subject axis was previously doing about a third of its job. Across the whole session the deck went 6,626 → 12,955.

**Tests run:** `npm test` — **345 pass, 0 fail, 0 skipped**, and three consecutive clean full-suite runs. Browser-verified at 360×640: no console errors; deck builds 12,955 items over 15 subject labels and 10 object labels; 0 coreferencing items and 0 items without four distinct options across the entire deck. Spot-checked newly reachable combinations — `הדלקתן אותן` (2fpl subject with 3fpl object, neither expressible before), `זריתי לך מלח על הפצעים` (`לזרות` newly linked, correct lamed-hey paal first-person past), `תדליקו אותה` (2fpl future).

**Risks / regressions to check:** (1) **34 paradigms is a lot of hand-pointed Hebrew.** The plain↔pointed skeleton guard reports zero mismatches, which catches typos where the two disagree, but it cannot catch a self-consistent yet wrong vocalisation. Highest-risk spots, all pattern-derived rather than sourced: the guttural-pe futures `תַּהֲפֹךְ`/`תַּהַפְכִי` and `אֶהֱרֹג`/`תַּהֲרֹג`, the lamed-hey hifil `הֶעֶלְתָה` and `הִטְּתָה`, and `לְמָרֵר`'s `מֵרֵר`. A read-aloud pass is worthwhile. `לבאס` from the previous entry remains the single least certain. (2) The 34 are conjugation-only by design; if any should also be Translation Match cards, remove the id from `TRANSLATION_HIDDEN_STARTER_VERB_IDS` and check the merged-pool duplicate test, which already caught `לשטוף`/`לשרוף` colliding with existing cooking-verb cards. (3) `PREPOSITION_INFLECTIONS` is now ragged — `le` and `et` carry ten object rows, the other fourteen carry eight. Deliberate and commented, but a future change that assumes uniformity should know. (4) The layout gate failed once during a full-suite run and has not reproduced in seven subsequent runs (four isolated, three full); that run was also the one where the 34 verbs were still Translation Match-enabled, so the pool was 34 cards larger. Worth watching rather than treating as settled — `task-log.md` documents this test being genuinely flaky before, on random Binyanim root selection. (5) In the future tense 2mpl and 2fpl share one Hebrew form (תדליקו), so those he2en items are dropped by the ambiguity check while their en2he counterparts remain — correct, but it means feminine-plural subjects appear more often in one direction than the other. (6) Still unresolved and now the only item left from the original audit: `level` is authored on all 81 idioms and read by nothing.


### 2026-07-31 17:20 EDT — Realign Conjugation+ with its mission: finish the migration, rebalance the pool, deliver the subject and object axes

**Requested:** Continuation of the repair entry above. With the migration defects fixed, bring Conjugation+ back in line with what it says it does — `app/bootstrap-data.js:145`, *"Conjugate both subject and object of Hebrew verbal idioms"*, reserved per `task-log.md:6357` for *"truly counter-intuitive expressions"*. Scope agreed with the user: finish the migration, rebalance the pool with new idioms rather than a lower bar, and close both axes including second person in past and future.

**What the mode was actually drilling.** Measured before starting: the pool was 69% one frame after the literal verbs came out; the subject axis had **no first person at all**, and second person existed only in the present, because an idiom's tense tables are keyed by gender and number (`msg/fsg/mpl/fpl`) and those four slots are third-person in past and future; the object axis was missing the feminine-singular addressee, which also stranded 36 authored `suffix_forms` strings. `level` is authored on every entry and read by nothing.

**Files changed:**

- `hebrew-idioms.js` — **Fixed two entries whose data lied before applying the migration criterion again.** `sider` (לסדר) and `marach` (למרוח) had `literal_*` repeating their own idiomatic gloss ("screws over", "strings along") instead of the literal reading, with `showMeaning: false`, so the prompt gave away the answer the feedback was supposed to reveal — and to a mechanical re-run of the filter they read as plain verbs, so both real idioms would have been deleted. Now `{s} arranges {o}` / `{s} smears {o}` with `showMeaning: true`. **Removed the last 8 literal verbs** (`itzben`, `hitish`, `shaga`, `hitrif`, `nitzel`, `bies`, `hitzchik`, `rima`) — every one passed the migration's own test that a `direct` entry whose literal reading equals its gloss teaches nothing the plain conjugation game doesn't. **Added 12 new idioms** so the pool was paid for rather than shrunk: 8 `direct` (`srifat_oto` לשרוף "blow someone's cover", `pitzutz_oto` לפוצץ "stand someone up", `gmirat_oto`, `shvirat_oto`, `harigat_oto`, `schivat_oto`, `prikat_oto`, `dchifat_oto` לדחוף "pull strings for someone") and 4 `possessive_suffix` (`haavara_al_midotav` על מידותיו, `hotzaa_migidro` מגדרו, `hotzaa_meashtonotav` מעשתונותיו, `haamada_al_tivo` על טיבו). All satisfy the `task-log.md:242` criterion — an animate subject that conjugates, and a meaning not derivable from the parts. Pool 69 → 81; `direct` 9 → 17, `possessive_suffix` 12 → 16, `l_dative` share **69.6% → 59.3%**. New entries are `niqqud_status: "unreviewed"`, so they render plain, per the existing contract; no pointing was invented for them.
- `hebrew-verbs.js` — 11 new fully pointed paradigms, 186 → 197. The 8 migrated verbs, each modelled on an in-repo paradigm of the same shape rather than pointed from scratch: `לעצבן` on `לבלבל` (quadriliteral piel), `להתיש` on `להציל` (the geminate hifil surface matches pe-nun), `לשגע` (final-ayin piel, furtive patach), `להטריף`/`להצחיק` on `להפחיד`, `לנצל`, `לבאס` on `לברר` (א as R2 blocks the dagesh, so hiriq→tsere), `לרמות` on `לנסות` (lamed-hey piel). Plus 3 high-leverage paradigms chosen because they unlock the most idioms per paradigm: `להוציא` (9 idioms), `לתפוס` (4), `להעמיד` (3).
- `app/constants.js` — **`ADV_CONJ_OBJECTS` is now derived from `PREPOSITION_INFLECTIONS`** instead of hand-copied beside it. `dirObj` is the `et` row and `lObj` is the `le` row; maintained separately they had drifted, and this table was missing `2fsg` entirely, so a learner met לָךְ and אוֹתָךְ in Prepositions and never here. 7 → 8 objects. `2msg` is relabelled "you (m.sg.)" because "you (sg.)" is ambiguous once the feminine exists — the same ambiguity discipline as `task-log.md:645`. **`ADV_CONJ_SUBJECTS` rebuilt with person**: 8 → 12 entries covering first, second and third person, each carrying `personKey` plus `pastSlot`/`futureSlot` naming the hebrew-verbs paradigm slot. The `tenses: ["present"]` gate is gone; the restriction now lives in form resolution, which can consult a paradigm.
- `app/adv-conj.js` — new `resolveAdvConjVerbForm(idiom, subject, tense)`. Present reads the idiom's own gender/number table, since Hebrew present does not inflect for person and every subject is valid there. Past and future read a person-marked slot from the verb's paradigm in `hebrew-verbs.js`, joined on the idiom's `verb` lemma — the same join `PREPOSITION_VERB_LINKS` introduced in `8d95686`. An idiom whose verb has no paradigm degrades to third person in those tenses, exactly as the file behaved before, rather than disappearing. Both answer builders now take the subject entry instead of `(subjectForm, subjectPronoun)`; the pronoun argument was never read. `isSecondPersonSubject`/`isSecondPersonObject` replaced by `subjectCoreferencesObject`, the same rule as `app/prepositions.js:78` — blocks identical pairs and same-person 1st/2nd collisions across number, allows disjoint third person, so "I annoy me" cannot generate while "he annoys him" still can. The he2en ambiguity check and the subject-label collapser now compare resolved forms, so "I (m.)"/"I (f.)" split in the present and merge to "I" in the past automatically. Paradigm lookup is memoized but **never caches an empty map** — it can run before the runtime has its `verbApi`, and caching that would silently disable the join for the session.
- `index.html` — `app/constants.js` cache-busted to `?v=20260731a`, joining the four data files and `adv-conj.js` already bumped in the repair entry.
- `tests/hebrew-idioms.test.js` — **restored both weakened thresholds**: pool floor `>= 75` → `>= 80` and `l_dative <= 0.7` → `<= 0.6`, each with a comment on why it was relaxed and why it is back. Counts 77 → 81, unreviewed 72 → 76. Object-key list gains `2fsg`. Contexts now load `preposition-data.js` before `constants.js`, which is a real dependency now rather than an implicit one. New test pinning the join's person coverage with an explicit floor, so the gap is measured instead of assumed.
- `tests/app-progress.test.js` — replaced the test that pinned "present-tense you forms without extending past/future", whose premise this change inverts, with two tests: one asserting the subject list spans three persons and that `getAdvConjSubjectsForTense` no longer gates on tense, one asserting past/future reach beyond third person **only** when a paradigm backs the idiom and stay third-person-only when none does. Added `getSeedVerbEntries` to the harness's verb-API stub — without it the join silently collapses in tests, which is how the second test caught the empty-map caching bug. Updated the four "you (sg.)" label expectations.
- `tests/hebrew-verbs.test.js` — entry count 186 → 197.

**Behavior changed:** Conjugation+ deck 6,626 → **8,400 items**. The learner can now be asked to conjugate **first person** (אני / אנחנו) in every tense and **second person in past and future** — `הדלקתי אותך` ("I lit you up") was not expressible before. 43 of 81 idioms have full person coverage in past and future; the other 38 keep third person there. The feminine-singular addressee (לָךְ / אוֹתָךְ) is drillable for the first time, and the `2fsg` suffix forms authored on all 16 `possessive_suffix` entries are reachable. The pool is 81 genuine expressions with no plain verbs left in it, and no frame above 60%.

**Tests run:** `npm test` — **345 pass, 0 fail, 0 skipped** (343 at the end of the repair entry; +2 net after replacing one test with two and adding a coverage guard). Browser-verified at 360×640 on `ulpango-dev`: no console errors; deck builds 8,400 items with 8 objects and 15 distinct subject labels across three persons; a forced past first-person item renders "I lit you (m.sg.) up" → `הדלקתי אותך` with all three distractor axes intact (`הדלקתי אתכם` wrong object, `הדליקו אותך` wrong subject, `הדלקנו אותי` both); `body.scrollHeight` 640 against the 640 budget.

**Risks / regressions to check:** (1) **`לבאס` is the one paradigm I would want a native-speaker eye on.** It is slang with no settled normative pointing; the forms follow `לברר`'s middle-guttural piel pattern (בֵּאֵס / מְבָאֵס), which is the right derivation, but it is a derivation rather than a sourced form. The other ten follow in-repo models of the same shape. (2) 38 of 81 idioms are still third-person-only in past and future; the join floor test records this. The cheapest wins remaining are `לשבור`, `לגנוב`, `להרים`, `להעביר` at 2 idioms each. (3) The 12 new idioms are unpointed, so they show plain Hebrew even with niqqud on — correct per the contract, but it widens the unreviewed share to 76 of 81. (4) `ADV_CONJ_OBJECTS` now throws if `preposition-data.js` has not loaded; `index.html` and both test harnesses order it first, but any new context loading `app/constants.js` alone will fail loudly rather than silently drift. (5) `2fpl`/`3fpl` objects are still unreachable because `PREPOSITION_INFLECTIONS` has no אתכן/אותן rows — adding them means extending the shared table, which also grows the Prepositions deck. (6) Present-tense he2en items remain almost entirely suppressed, since a person-neutral present form rarely names one subject; this predates the change but the resolved-form comparison makes it explicit. (7) `level` is still authored on all 81 entries and read by nothing — either wire it into selection or delete it. (8) Unrelated and pre-existing: the app shell reports 383px of horizontal content at a 360px viewport on the home screen with no game running, so it is not mode-specific; the repo's layout gate measures vertical scroll only.


### 2026-07-31 14:40 EDT — Repair the Conjugation+ idiom migration (Phase 0 of the realignment)

**Requested:** Audit commit `4e8dc9b` ("Migrate 22 straightforward verbs from idioms to hebrew-verbs conjugation drills"), which pulled plain verbs out of Conjugation+ because that mode is for expressions that cannot be read literally. Then measure what Conjugation+ still drills against its stated mission. This entry covers the repair phase only; the mission-alignment work (finishing the migration, rebalancing the pool, the subject/object axes) is planned and not yet started.

**Verdict on `4e8dc9b`: the editorial call is right and the migration stands.** All 23 removed entries were `object_type: "direct"` whose `literal_sg` merely paraphrased their own `english` gloss, so they taught nothing the plain conjugation game doesn't. Niqqud on the hard cases is correct (`הֶעֱלִיב` guttural pe, `מְשַׁכְנֵעַ`/`מַרְגִּיעַ` furtive patach, `מַצִּיל` pe-nun assimilation, `עוֹדֵד` polel). But the commit is a squash of ~8 unrelated sessions and shipped six defects, all repaired here.

**Files changed:**
- `hebrew-verbs.js` — **(1) Wrong Hebrew.** `advanced-verb-lehargia` had 7 plain forms that lost the ר: past `הגעתי/הגעת/הרגנו/הרגתם/הרגתן` and imperative `הגע`. With niqqud off (the default) the past of "to calm down" drilled as `הרגנו` = *we killed* and `הגעתי` = *I arrived*. Corrected against the neighbouring final-ayin hifil `lehaftia`. `advanced-verb-lehamshikh` past 2fs pointing `הִמְשַׁכְּ` was truncated → `הִמְשַׁכְתְּ`. A consonantal-skeleton sweep of all 190 entries found exactly these 8, all introduced by that commit. **(2) Duplicates.** Deleted the five entries the commit added that already existed: `advanced-verb-lehasbir` and `advanced-verb-lehaskim` were outright **id collisions**, and `lehamshikh`/`lehaamin`/`lehatchil` duplicated `common-verb-*`/`character-verb-*` lemmas. Kept the pre-existing entries untouched, so this is a pure de-duplication with no coverage change. This also restores `TRANSLATION_HIDDEN_STARTER_VERB_IDS`, which the new ids were bypassing — להתחיל and להמשיך are deliberately hidden from Translation Match and were visible again. **(3)** Restored `להעריך` as `advanced-verb-lehaarich` (hifil, ע.ר.כ, guttural pe), the one verb of the 23 that was removed from the idioms and re-homed nowhere; registered in the hidden-verb set so it does not compete with the pre-existing `["to assess", "להעריך"]` card. 186 seed entries.
- `vocab-data.js` — **(4) 64 orphaned card ids.** Ids are positional (`${category}-${idx+1}-${slug}`, line 2282), and the commit parked verbs at the **head** of `emotional_psychological_expanded` (17), `communication_mastery_expanded` (3) and `everyday_survival_expanded` (2), plus `ניתוק`/`אגף` mid-array in `scientific_analytical`. That renumbered 64 pre-existing cards and stranded their Leitner box, accuracy history and mastered toggle. The file passed the *letter* of append-only — `git show --numstat` reported `35 0`, zero deletions — which is why nothing caught it. Removed the 22 parked verb rows (redundant with `hebrew-verbs.js`) so numbering reverts exactly, and re-filed `ניתוק`/`אגף` as a **tail append** on `politics_society_expanded`, which is where their sentences actually sit. Diffing generated ids against `4e8dc9b^`: **0 orphaned**.
- `app/adv-conj.js` — **(5) Dead code.** Removed the `"infinitive"` tense branches (`tenses`, `infinitive_tense`, `infinitive_tense_niqqud`, `literal_infinitive`) and the `invalid_objects` distractor filter. Zero rows in `hebrew-idioms.js` used any of them: `lehakir_li` was added and removed inside the same commit. The feature never worked regardless — the normalizer at `hebrew-idioms.js:2070-2075` aliases `present_tense`/`past_tense`/`future_tense` but never `infinitive_tense`, so the branch always read `undefined`. No behavior change.
- `hebrew-idioms.js` — stripped the 23 whitespace-only lines left where entries were blanked rather than removed.
- `sentence-bank-data.js` — the three new לתפוס rows (`everyday_147/148/149`) shipped 3 Hebrew and 3 English distractors against a floor of 4; added one pointed distractor per axis. Stripped trailing punctuation from the chips of all 6 rows the commit added (`everyday_146/147/148/149`, `inbal_96`, and the row below) — 621 of 627 entries strip it, these did not, so chips read `"משקר,"` and `"הודה בזה."`. Renamed `everyday_145` → `professional_97`: it carried `category: "professional"`, the only id-prefix/category mismatch in 627 entries, and the content (a business relationship) is professional. Normalized `הַפִּינַנְסִי` → `הַפִינַנְסִי` in `professional_93`'s chip; פיננסי is a loanword pronounced /f/, and the dagesh-less form is what the sentence's own `hebrewNiqqud` and the vocab card use.
- `index.html` — **(6) Cache-busting.** The commit edited five `.js` files and bumped **zero** `?v=` strings, so none of the migration was live. All five bumped to `?v=20260731a`; `__build` markers in `vocab-data.js`, `hebrew-verbs.js` and `sentence-bank-data.js` bumped to match.
- `tests/gameplay-layout.test.js` — un-skipped `compact gameplay and safe centering hold in rendered Chrome`. It had been turned into `test.skip` to dodge a sandbox `EPERM` on `127.0.0.1`, disabling the headless-Chrome gate for the 360×640 no-scroll budget across all nine modes. The paired `127.0.0.1` → `localhost` bind change in the same commit is the actual fix; with it kept, the test passes.
- `tests/hebrew-verbs.test.js` — two new guards. A **plain↔pointed consonantal-skeleton** assertion over every stored form (ktiv male means the pair may differ by ו/י and nothing else) — this is the test that would have caught defect 1. A **unique id and lemma** assertion — there was none, which is why the two collisions shipped. Count 190 → 186; removed the stale bundled edits.
- `tests/vocab-data.test.js` — new **append-only id baseline** (`tests/fixtures/vocab-id-baseline.json`, 1,693 ids) asserting no id ever disappears; count assertions cannot see a head insertion, since it changes no total. New **merged-pool uniqueness** test over `getBaseVocabulary()` + `getSeedVocabularyEntries()` as `app.js:616-619` actually concatenates them — every existing uniqueness test loads only `vocab-data.js`, which is how 25 duplicate playable cards shipped green. Bounded the politics tranche slice to `(18, 168)` so it stays pinned at its 150 cards while the category accepts tail appends.
- `tests/sentence-bank-data.test.js` — new guard that register-bucket sentence ids agree with their category. Corrected the title that said 618 while asserting 627.
- `tests/hebrew-idioms.test.js` — removed the unreachable infinitive branches that mirrored the deleted `adv-conj.js` code.
- `docs/product-roadmap.md` — corrected "100 idioms" → 77 (two sites) and the vocabulary count.

**Behavior changed:** The regular conjugation game no longer teaches `הרגנו` ("we killed") as the past of להרגיע. Translation Match loses 25 duplicate cards (35 → 10; the remaining 10 pre-date this commit). להתחיל and להמשיך are hidden from Translation Match again. 64 vocabulary cards keep the progress that was silently orphaned. להעריך is drillable again, in the conjugation game. Conjugation+ itself is unchanged — the removed code was unreachable. **And all of the migration reaches the browser for the first time, since the cache-bust was missing.**

**Tests run:** `npm test` — **343 pass, 0 fail, 0 skipped** (baseline before this session: 337 pass, 1 skipped). Browser-verified at 360×640 and 375×812 on `ulpango-dev`: no console errors; Conjugation+ renders `לתפוס מישהו בקלקלתו` in the future with all three distractor axes intact (two wrong-object, one wrong-subject), grades correctly and shows "Everyday meaning: to catch someone red-handed"; `body.scrollHeight` 640 against a 640 budget with the feedback tray open. Confirmed in the running app that the live deck now serves `הרגענו`/`הרגעתי`/`הרגע`, that duplicate playable pairs are down to the 10 pre-existing, and that להעריך exposes 24 forms.

**Risks / regressions to check:** (1) `tests/fixtures/vocab-id-baseline.json` must be regenerated in the same commit as any legitimate tail append, and the diff checked to be additions only — otherwise the guard goes stale. (2) `KNOWN_MERGED_DUPLICATES` pins 10 pre-existing duplicate pairs (`core_advanced-*` vs `advanced-verb-*`); they cannot simply be deleted, because those rows sit mid-category and removing them would orphan ids exactly as this entry repairs. Retiring one means setting `translationQuiz: false` on its `vocab-data.js` row so the id survives; the test fails if the list goes stale in either direction. (3) `ניתוק`/`אגף` changed ids when re-filed — safe only because the cache-bust was missing, so they were never live. (4) Deleting the five duplicate `hebrew-verbs.js` entries drops a second `להאמין` deck card that had imperative forms; the surviving `character-verb-lehaamin` has none. Adding imperatives to להאמין and להסכים is worth doing deliberately, but it changes their deck shape from 21 to 24 forms and a pinned test asserts 21. (5) `colloquial_10` and `colloquial_18` are still below the English distractor floor — pre-existing, untouched.

**Still open (planned, not done):** finishing the migration (8 `direct` entries — `itzben`, `hitish`, `shaga`, `hitrif`, `nitzel`, `bies`, `hitzchik`, `rima` — still pass the commit's own literal test); rebalancing the pool, since removing them takes `l_dative` from 62% to 70% and two thresholds in `tests/hebrew-idioms.test.js` were relaxed (`0.6` → `0.7`, floor `100` → `75`) rather than re-scoped; the missing `2fsg` object in `ADV_CONJ_OBJECTS`, which diverges from `PREPOSITION_OBJECTS` and leaves 36 authored suffix strings unreachable; and person-marked past/future so the mode delivers the subject axis its tagline promises. **Before re-running the migration filter, fix three mis-authored `literal_*` fields** — `sider` and `marach` repeat their idiomatic gloss instead of the literal reading, so a mechanical pass would delete real idioms; `hitzchik` is the reverse.



### 2026-07-31 11:23 EDT — Fix English translation for sentence colloquial_132 ("these matkot")

**Requested:** Correct English translation for `colloquial_132` in sentence-bank-data.js from "the matkot" to "these matkot" to properly match Hebrew "המטקות האלה".

**Files changed:**
- `sentence-bank-data.js` — Updated `english` and `englishTokens` of `colloquial_132` to use `"these matkot"` instead of `"the matkot"`.

**Behavior changed:** The English prompt in Sentence Bank for `colloquial_132` now accurately reads "It's impossible to sleep on the beach, these matkot are everywhere." matching "המטקות האלה".

**Tests run:** `npm test` (337 pass, 1 skipped, 0 fail), `node --test tests/gameplay-layout.test.js`.

**Risks / regressions to check:** None expected.
...
### 2026-08-02 20:40 EDT — Delete merged branches, fix two real layout overflows, and correct the new niqqud

**Requested:** Explain and carry out remote-branch cleanup if it is proper housekeeping; find and fix the intermittent layout-test failure; and check the niqqud on the recently authored Hebrew.

**Files changed:**
- `styles.css` — two mitigations for feedback-tray overflow at 360×640. Compaction for `.lesson-shell.mode-adv-conj .feedback-tray` (padding `0.34rem 0.6rem 0.36rem`, `line-height: 1.28`), copying the Binyanim block above it. Plus capped scrollable trays in the `max-height: 700px` query, following the Sentence Bank precedent: `5.5rem` for Conjugation+ and `4.5rem` for Binyanim, which needs the tighter value because its prompt card (~88px) and four glosses (~221px) make its shell ~34px taller before any feedback renders.
- `tests/gameplay-layout.test.js` — three fixes. (1) New `measureGeometry` helper awaits `document.fonts.ready`, all running animations, and two frames before measuring; all six geometry measurements now route through it. The old code measured the tray mid-`feedbackTrayIn`, which the original failure payload proves: `feedback.bottom − footer.bottom` was exactly 6.000px, the animation's unstarted `translateY(6px)`, and a transformed descendant still contributes to an ancestor's scrollable overflow in Chrome. (2) The Conjugation+ block now pins its draw — stubbing `Math.random` and `utils.weightedRandomWord`, then restoring both, mirroring the Shema block — and deliberately answers wrong so the longer "לא בדיוק" prefix is measured. (3) Teardown now waits for Chrome to exit before removing its profile directory and gives `rmSync` `maxRetries`.
- `sentence-bank-data.js` — 32 holam corrections plus 24 other pointing fixes (see below).
- `vocab-data.js` — 9 pointing fixes. No `he`, `en`, or `id` field changed.
- `tests/sentence-bank-data.test.js` — two new tests: an **un-allowlisted** consonantal-skeleton check over every sentence, token, distractor, and alternate (the existing one is scoped to `POLITICAL_ENTRY_IDS`, which is how 90 `idan_` rows shipped with nothing stronger than "one mark exists somewhere"), and a mechanical-fault gate for holam-before-vav, dagesh on א/ח/ע/ר, and vowels on final letters.
- `tests/vocab-data.test.js` — the same two gates for the vocabulary deck.
- `index.html` — `vocab-data.js`, `sentence-bank-data.js`, and `styles.css` bumped to `?v=20260802h`; `__build` matches.

**Behavior changed:**
- **Branches.** Deleted 16 fully-merged remote branches and 15 local ones. The remote now holds `main` plus the three genuinely unmerged `agent/*` branches. A 17th, `mission-quit-and-review-character-picker`, was already gone on GitHub — `git fetch --prune` cleared the stale ref. Local `conjugation-feedback-and-content` survived because `git branch -d` correctly refused it: it carries one unmerged June commit ("Fix mobile/tablet scroll and viewport issues on iOS"). The two `claude/*` worktree branches were skipped.
- **Layout.** Conjugation+ and Binyanim feedback no longer overflow the 488px body at 360×640. Measured on the pinned worst-case Conjugation+ question: tray content 86px inside an 88px box, `bodyOverflow: 0`, nothing clipped. Binyanim's median feedback is 31px and the tallest in a sampled draw was 70px, both under its 72px cap; only the rare ~106px teaching notes now scroll internally, which is the behaviour Sentence Bank has had since `task-log.md:5421`.
- **Niqqud.** 41 pointing corrections. No routing, count, or id changed — `npm run report:characters` is byte-identical.

**Niqqud detail:**
- **32 holam-before-vav**, all in `idan_` rows and zero elsewhere in 18,558 pointed pairs in the repo: `לִשְׁטֹוף` → `לִשְׁטוֹף`, `טֹופֶס` → `טוֹפֶס`, `לִבְדֹּוק` → `לִבְדּוֹק` and 20 more distinct forms across 20 rows. Invisible to every prior check because both spellings carry marks and the skeleton helper strips vav. `לַחְבֹּוש` also gained its missing shin dot.
- **5 self-contradictions** in the new content: `דיווח`, `למפקד`, `לחייל`, `במסדר`, `מנות`.
- **Confirmed word errors:** `תלונה` and `שישי` had spurious dagesh; `מסקנות` had the wrong plural stem; `מטען` on the charger card was pointed `מַטְעָן` (cargo) instead of `מַטְעֵן`; three `devices_os_apps` screen compounds used the absolute `מָסַךְ` where the construct `מְסַךְ` belongs; `מצב כהה` was aligned with `מצב טיסה`.
- **6 pre-existing errors** the cross-check surfaced by conflict: `everyday_82`/`_86` were missing a post-article dagesh, `colloquial_109` had the wrong construct, and `colloquial_76`, `inbal_43`, `inat_11`, `technology_ai-017`, `media_digital_life_expanded-023`, `literature_arts_cultural_history-030` each had a pointed form that dropped a letter or used a mismatched geresh.

**Tests run:**
- `npm test` — **369 pass, 0 fail** (365 before, plus 4 new gates).
- Layout test isolated: **1 failure in 12 runs before the caps** (Binyanim, 493px > 488px), then **0 in 24 runs** after the split caps and teardown fix. Against the combined pre-fix rate of roughly 1 in 7, zero in 24 is about a 2% coincidence.
- The intermediate 5.5rem-for-both attempt is what produced the decisive measurement: `feedback: {clientHeight: 86, scrollHeight: 106}` with the body still 8px over, proving the cap engages and that Binyanim needed a tighter one.
- `npm run report:characters` — identical to before, and a field-level diff against `HEAD` confirmed 0 changes to `id`, `he`, or `en` and 9 to `heNiqqud`.
...
### 2026-08-03 17:52 EDT — Freeze and publish the final Inbal-grain five-character cast

**Requested:** Install the user-approved Inat, Ivri, and Idan redraws, preserve Ido and Inbal byte-for-byte, make every character use the same effective pixel-art resolution contract, organize the image sources, push everything to GitHub, merge it into `main`, and prevent GitHub Pages from serving the superseded sprites.

**Files changed:**
- `assets/{inat,ivri,idan}/*.png` — installed the 18 approved 512×512 RGBA production reactions under the existing runtime filenames.
- `assets/sprite-masters/{inat,ivri,idan}/*-transparent.png` — installed the 18 approved 1254×1254 transparent masters; the new Ivri and Idan directories bring the tracked master set to 30 files.
- `scripts/build-{ivri,idan}-sprites.py` and `scripts/build-approved-sprites.py` — moved Ivri and Idan to the same direct 1254→512 nearest-neighbor deterministic export used by the rest of the cast and made the aggregate builder cover all five characters. Idan's old 128px/96-color reduction was removed.
- `scripts/audit-sprites.py`, `assets/sprite-lock.json`, `tests/sprite-lock.test.js`, and `tests/character-mission.test.js` — added the visible-only 32/64-color multiscale spatial detector, locked exact hashes and spatial signatures for all 30 masters and 30 outputs, enforced direct builders and anti-128/256-round-trip guards, and removed the obsolete assertion that required Idan's coarse pipeline.
- `assets/SPRITE_STANDARD.md`, `docs/sprite-prompts/final-inbal-grain-standard.md`, `docs/sprite-prompts/{ido,inat,inbal}.md`, and `docs/character-gameplay-strategy.md` — made Inbal the sole visual style/grain authority, preserved the exact reference order and hardcoded generation/rejection rules, and removed contradictory Ivri-authoritative and logical-canvas guidance.
- `styles.css` and `index.html` — changed the stylesheet and all 30 sprite URLs to the new `20260803b` cache key so the deployed HTML cannot select an older cached stylesheet or sprite.
- `task-log.md` — this record.
- Local cleanup — moved the obsolete ignored `assets/{ivri,idan}/source/` folders into the ignored `tmp/imagegen/archive/` staging archive. The tracked `assets/sprite-masters/` tree is now the only authoritative master location.

**Behavior changed:** Inat, Ivri, and Idan now use their approved fine-grain complete reaction sets throughout the app. All five builders use the same direct deterministic export contract, with Ido's centered shirt mark remaining the sole post-export exception. Existing reaction names and runtime interfaces are unchanged. GitHub Pages receives a new stylesheet cache key and one shared new sprite cache key for all 30 images.
...
### 2026-08-08 20:54 EDT — Remove magenta spill from the full character sprite set

**Requested:** Clean the visible magenta key-color fringe from all five characters × six reactions in one deterministic batch; stage candidates under `tmp/sprite-edge-cleanup/`; add a reusable non-destructive maintenance command and broader audit; visually approve and promote the batch; refresh the sprite lock and cache key; preserve the canonical checkout, legacy compatibility symlink, and unrelated `ivritelite` files.

**Files changed:**
- `scripts/clean-sprite-edges.py` and `scripts/sprite_edges.py` — added the reusable 30-master cleanup pipeline, conservative exterior-spill detector, per-file removal and runtime-silhouette limits, 60-file validation, report generation, black/white contact sheets, and explicit `--apply` promotion mode.
- `scripts/build-approved-sprites.py` and `scripts/build-{ido,inbal,ivri,inat,idan}-sprites.py` — added configurable source/output roots while retaining the tracked defaults, so candidate production sprites use the exact direct 1254→512 nearest-neighbor builders and Ido logo composition; aggregate builds now run the five independent character builders concurrently.
- `scripts/audit-sprites.py`, `tests/sprite-lock.test.js`, `tests/character-mission.test.js`, and `assets/sprite-lock.json` — extended the frozen sprite audit to reject edge-connected magenta-like spill, covered the new source-root contract, refreshed all 60 approved hashes/signatures intentionally, and advanced the cache-key assertions.
- `assets/sprite-masters/{ido,inbal,ivri,inat,idan}/*-transparent.png` and `assets/{ido,inbal,ivri,inat,idan}/*.png` — promoted the reviewed 30 cleaned 1254×1254 RGBA masters and rebuilt the corresponding 30 512×512 RGBA production sprites.
- `package.json` and `assets/SPRITE_STANDARD.md` — added and documented `npm run sprites:clean-edges`, its non-destructive default, candidate layout, validation gates, contact-sheet review, and explicit apply workflow.
- `styles.css` and `index.html` — advanced the shared stylesheet and all 30 sprite cache URLs from `20260803b` to `20260808a`.
- `task-log.md` — this record.

**Behavior changed:** The 30 production character reactions no longer carry the visible exterior magenta fringe. Gameplay APIs, reaction names, dimensions, alpha mode, nearest-neighbor geometry, and runtime behavior are unchanged. Future edge cleanup defaults to ignored candidate output and cannot alter tracked assets without `--apply`.

**Tests run:** Baseline `npm test` — **380 pass, 0 fail, 0 cancelled**. Baseline `npm run sprites:audit` — **pass: 30 production files, 30 tracked masters, and 30 deterministic rebuilds**. Candidate `npm run sprites:clean-edges` — **pass: 30 cleaned masters and 30 production candidates**. Automated candidate validation — **60/60 RGBA files at required dimensions with binary alpha, preserved corner behavior, zero detectable exterior magenta-like pixels, maximum visible-master removal 0.3943%, and maximum runtime bounding-box shift 1 pixel**. Black and white 30-reaction contact sheets — **visual pass: no remaining hot-magenta fringe, holes, or meaningful lost detail**. Promotion `npm run sprites:clean-edges -- --apply` — **pass**. `npm run sprites:audit -- --write-lock` — **pass and lock intentionally refreshed**. Final `npm run sprites:audit` — **pass, including two byte-identical independent direct production rebuilds**. The first post-cache-key `npm test` exposed only the pinned old cache-key expectation; after advancing that assertion, final `npm test` — **380 pass, 0 fail, 0 cancelled**. `python -m py_compile scripts/clean-sprite-edges.py scripts/sprite_edges.py scripts/audit-sprites.py scripts/build-approved-sprites.py scripts/build-ido-sprites.py scripts/build-inbal-sprites.py scripts/build-inat-sprites.py scripts/build-ivri-sprites.py scripts/build-idan-sprites.py` — **pass**. Candidate-to-tracked byte comparison for all 60 promoted PNGs — **pass**. `git diff --check` — **pass**.

**Risks / regressions to check:** The magenta-like threshold is deliberately conservative and uniform across the cast; it removes only matching pixels within two master pixels of transparency and enforces the 0.4%/two-runtime-pixel safety ceilings. Very dark purple pixels that do not satisfy the key-spill signature remain as authored shading. The supported Codex `Edit project` UI still must be used to make `/Users/mikesexton/Documents/Ulpango` the saved Ulpango project's primary folder because Codex blocks Computer Use from automating its own window; no private project state was edited. The legacy `/Users/mikesexton/Documents/New project/Ulpango` symlink still targets the canonical checkout, and `/Users/mikesexton/Documents/New project/ivritelite` remains untouched.

### 2026-08-08 21:19 EDT — Plan the next large content expansion

**Requested:** Audit and plan a large addition covering more Binyan Board roots, new vocabulary across the cast with Hebrew smartphone-interface terminology for Ivri, more sentences with vocabulary-coverage and character-balance goals, and conjugation paradigms for technology verbs such as upload.

**Files changed:**
- `task-log.md` — recorded the planning audit and proposed scope. No production content or runtime files were changed.

**Behavior changed:** None. The audit measured 2,108 vocabulary cards, 767 sentences, 231 conjugation seed paradigms producing 247 sense-specific deck items, and 60 Binyan Board roots with 297 authored forms. Current owned sentence counts are Ido 202, Inbal 96, Ivri 138, Inat 162, and Idan 115; a conservative exact/clitic-normalized scan found explicit sentence support for 523 of 2,108 vocabulary headwords, including 14 of 114 cards across Ivri's three dedicated technology shelves. Official Hebrew Apple and Google help pages were reviewed for current interface language around settings, updates, backup/restore, synchronization, permissions, verification, notifications, upload, and download.

**Tests run:** None — this was a read-only content and architecture audit. `npm run report:characters` was run successfully and reported the current routed and drawable pool counts; several read-only Node audit snippets were also run to measure Binyan, conjugation, sentence-audience, and conservative vocabulary-to-sentence coverage.

**Risks / regressions to check:** The 24.8% global and 12.3% technology coverage figures are deliberate lower bounds, not semantic truth: they count normalized headword surfaces and common attached clitics but do not infer plural, construct-state, or conjugated forms. Before turning the plan into a coverage gate, implement an auditable three-state join (exact target support, reviewed morphological/alias support, unsupported) instead of stemming Hebrew automatically. Sentence parity must use routed ownership and drawable audience, not only id prefixes; Ido and Ivri own register banks without character prefixes. Keep vocabulary rows append-only because ids encode category position, keep all new sentences below the reviewed marker, and author every conjugation and Binyan form rather than generating unverified Hebrew.

### 2026-08-08 22:03 EDT — Implement the large cast and technology content expansion

**Requested:** Implement the planned addition: more Binyan Board material, new vocabulary across the cast with a substantial Hebrew smartphone-interface shelf for Ivri, sentences that explicitly test the new vocabulary while improving the character spread, and full conjugation paradigms for technology verbs such as upload.

**Files changed:**
- `vocab-data.js` — appended 60 pointed, playable cards without changing existing ids: 40 smartphone-interface terms on Ivri's `devices_os_apps` shelf and five cards each for Ido (`media_digital_life_expanded`), Inbal (`religious_life_practice`), Inat (`literature_arts_cultural_history`), and Idan (`emergency_response`). Build key advanced to `20260808b`.
- `sentence-bank-data.js` — appended 50 reviewed rows below the append-only marker: `professional_123`–`152` exercise all 40 Ivri technology cards, and five rows apiece exercise each cast addition (`colloquial_163`–`167`, `inbal_97`–`101`, `inat_26`–`30`, `idan_116`–`120`). Every row has pointed chips, four distractors, bilingual chip parity, an explicit word-order decision, and authored alternates where a neutral reordering is independently idiomatic. Build key advanced to `20260808b`.
- `hebrew-verbs.js` and `app/character-data.js` — added ten authoritative present/past/future paradigms for להתקין, להסיר, למחוק, לשתף, לסנכרן, לגבות, לשחזר, להתחבר, להתנתק, and לאפס; split להעלות and להוריד into separate raise/upload and take-down/download deck senses; routed the technology senses and paradigms to Ivri; kept duplicate infinitives out of Translation Match while retaining them for conjugation and sentence hints.
- `verb-game-data.js` — added 12 authored roots and 62 forms across the seven Binyan slots, taking the board from 60 roots/297 forms to 72 roots/359 forms; data version advanced to 1.1.
- `scripts/content-coverage-report.js` and `package.json` — added `npm run report:coverage`, an auditable three-state exact/reviewed/unsupported vocabulary-to-sentence report with conservative ordinary-clitic matching and no automatic stemming.
- `tests/content-coverage.test.js`, `tests/vocab-data.test.js`, `tests/sentence-bank-data.test.js`, `tests/hebrew-verbs.test.js`, `tests/verb-game-data.test.js`, and `tests/fixtures/vocab-id-baseline.json` — pinned the new totals and semantics, registered compact glossary units, extended sentence alignment coverage, proved all 60 new cards have exact sentence support, protected the new sense split and paradigms, widened the Binyan expectations, and appended the new stable vocabulary ids to the fixture.
- `index.html` — advanced cache keys for all changed content and routing scripts to `20260808b`.
- `task-log.md` — this record.

**Behavior changed:** The live pool is now 2,168 vocabulary cards, 817 sentences, 241 conjugation paradigms producing 259 sense-specific deck entries, and 72 Binyan roots with 359 authored forms. Ivri gains a practical Hebrew smartphone vertical covering settings, accessibility, display, notifications, battery, storage, connectivity, account actions, and common device verbs. Every character gains five new thematic cards and five sentences; Ivri additionally gains the 40-card/30-sentence technology tranche. Owned sentence counts are now Ido 207, Inbal 101, Ivri 168, Inat 167, and Idan 120. Conservative exact/clitic sentence support rose from 523/2,108 (24.8%) to 600/2,168 (27.7%); the three Ivri technology shelves rose from 14/114 (12.3%) to 63/154 (40.9%), and every newly added card is exact-supported.
